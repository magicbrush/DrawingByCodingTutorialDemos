...
function draw() {	
	...
	for(var theta = 0;
		...
		theta += TWO_PI/3)
	{
		var rScale = 100 * sin(secs) + 80;
		var radius = 40 * cos(0.5*secs) + 70;
		var offsetX = sin(theta + secs) * rScale;
		var offsetY = cos(theta + secs) * rScale;
		drawSpiralingConfuseFace(
			centerX + offsetX, 
			centerY + offsetY, 
			radius);
...
function drawSpiralingConfuseFace(x,y,radius)
{
	var msecs = millis();
	var secs = msecs/1000;
	var cnt = 0;
